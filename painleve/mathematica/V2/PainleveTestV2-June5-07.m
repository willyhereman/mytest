...
If[debugwilly, 
   Print["At pt. SIMP3, after rule and after simplify, equations = "];
...
(* WH:01/31/2006 Block replaced by Module *)
  Module[{ initialConstantsOfIntegrationDebug = (* The debug Boolean. *)
    ...
    If[( Verbose /. {options} /. Options[PainleveTest] ),
      Print["The system (determining the first constant of integration) " <> 
    ...
    If[Length[ uSoln0 ] === 0,
      StylePrint[
        "The dominant behavior, " <> ToString[CleanUpFunction[ alphaSoln ]] <>
  ...
  Module[{ systemGenerationDebug = (* The debug Boolean. *)
    ...
    If[ Verbose /. {options} /. Options[PainleveTest],
      Print["Computation of THE RESONANCES for VARIOUS DOMINANT BEHAVIORS " <>
            "(one at the time)."];
      Print["START OF NEW BRANCH OF THE COMPUTATIONS."];
      Print[
  ...
  Module[{ constantsOfIntegrationDebug = (* The debug Boolean. *)
    ...
    If[ Verbose /. {options} /. Options[PainleveTest],
       Print["Computation of THE CONSTANTS OF INTEGRATION and COMPATIBILITY "<>
           "CONDITIONS for the various dominant behaviors (one at the time)."];
       Print["START OF NEW BRANCH OF THE COMPUTATIONS."]
    ];
    ...
    If[constantsOfIntegrationDebug,
      Print["The positive resonances are: ", resonanceList ];
    ...
    (* Solve for the (non-resonance) constants of integration. *)
...
constantsOfIntegration`KruskalSimplification[
  ...
  Module[{ KruskalSimplificationDebug = (* The debug Boolean. *)
    ...
    If[KruskalSimplificationDebug ,
      Print["The input (for the function constantsOfIntegration) is:"];
...
(* Looks like the condition is ignored. Needs further investigation *)
   Print["At pt. XXX1, det of righhandsideofmatrix = ",
         MapAll[Expand, Det[ rightHandSideOfMatrix ] ]
         ];
*)
...
(*
Print["At pt. XXX2, theSolution : ",
       CleanUpFunction[theSolution]
     ];
Print["At marker YYY2"];
*)
...
(* 
Print["At pt XXX3a, uList going into the IterativelySolve function, 
      uList :", CleanUpFunction[uList]
     ];
Print["At pt XXX3b, theSystem going into the IterativelySolve function, 
      theSystem :", CleanUpFunction[theSystem]
     ];
...
Print["At KKK2, condition based on zero replaced by sequence, testvalue: ", 
       CleanUpFunction[conditiontestvalue]
     ];
*)
...
*)
    ...
    If[ Length[uList] === 0, (* begin if1 *)
     (*
        Print["At pt. ZZZ3, because length of uList is zero: ", 
              CleanUpFunction[uList]
             ];
     *)
      Return[ 
    ...
    (* Sorts the equation by complexity. *)
    theSystem = 
     Sort[
      theSystem,
...
Print[" theEquation to be solved :" , 
      CleanUpFunction[theEquation]
     ];
*)
...
*)
    ...
    If[Length[ solveFor ] != 0, (* start if2 *)
       ...
       Print["At pt. ZZZ6, length solveFor is nonzero, length solveFor: ", 
              CleanUpFunction[Length[solveFor]]
            ];
       *)
       ...
       *)
      solveFor = 
        Flatten[{solveFor, Complement[uList, solveFor]}], (* else if2*)
        (* was ; when command was active, otherwise , *)
       (*
       Print["At pt. ZZZ8, length solveFor is nonzero, new solveFor: ", 
              CleanUpFunction[solveFor]
            ], (* was , when command was active *)
       *) 
...
(*
Print["At pt. LLL1, theSolution :", 
      CleanUpFunction[theSolution]
     ];
Print["At marker YYY6"];
Print["At pt. XXX6, theSolution :" , 
      CleanUpFunction[theSolution]
     ];
*)
...
Print["At pt. LLL2, before the joining starts, theSolution :", 
      CleanUpFunction[theSolution]
     ];
*)
...
Print["This is the inconsistent system with righthandside :"];
Print[CleanUpFunction[righthandside]];
Print["This is the inconsistent system with lefthandside :"];
Print[CleanUpFunction[lefthandside]];
*)
...
(* 
Print["At XXX11, this is uList: ", 
      CleanUpFunction[uList]
     ];
*)

    If[buildSystemDebug,
      Print["The system (within the constantsOfIntegration function) is:"];
      Print[CleanUpFunction[ theSystem ]];
...
Print["LATEST VERSION: May 31, 2007"];
Print["Version 2. First released: February 1, 2006."];
Print["Code last debugged by Baldwin on November 1, 2006."];
Print["Previous code update by Hereman on May 31, 2007."];
