...
(* Code last FIXED on June 30, 2018 at 19:10 by Hereman at home in Boulder *)
(* Search for FIX WH 06/30/2018 *)
(* Reasons:                     *) 
(* 1. Removed the Sort option of the Solve function -- no longer supported *)
(* 2. pdeForm does no longer work properly                                 *)
...
(* FIX 06/30/2018. Added spaces between the lines *)
PainleveTest::usage =
...
PainleveTest[
  ...
  Module[{ painleveTestDebug = False, (* The debug Boolean. *)
    ...
    If[ painleveTestDebug || (Verbose /. {options} /. Options[PainleveTest]),
      ...
      (* FIX WH 06/30/2018. following equation(s) *) 
      Print["The Painleve test is carried out for the following equation(s) in standard Mathematica notation: "];
      Print[
       TableForm[
       (* FIX WH 06/30/2018. *)
       (* replaced pdeForm[ # == 0 & /@ equations ] by *)
       Map[# == 0&, equations]        
       ]
      ];
      Print["or, in more readable subscript notation, used to shorten the output: "];
      (* Did not work. FIX WH 06/30/2018 *)
      (* 
      equationsprint = pdeForm[ equations ] /. {#[__] -> #}; 
      equationsprint = TableForm[ # == 0& /@ Map[ HideArgsWilly[#1]&, pdeForm[ equations ] ] ]; 
      Print["KKKKKKKKKKKKKKK 1, equationsprint : "]; 
      Print[equationsprint];
      Print["KKKKKKKKKKKKKKK 2, only pdeForm applied to equations : "];
      *)
      Print[
       TableForm[
        pdeForm[ # == 0& /@ equations ] 
    ...
    If[Length[variables] === 1,
      ...
      If[painleveTestDebug ||
        ...
        (*
          Print[
          TableForm[
          (* FIX WH 06/30/2018. pdeForm can no longer be applied. *)
          (* replaced pdeForm[ # == 0 & /@ CleanUpFunction[equations] ] by *)
          Map[# == 0&, CleanUpFunction[equations] ]  
          ]
        ]; 
        *)
       Print[
    ...
    If[painleveTestDebug || (Verbose /. {options} /. Options[PainleveTest]),
      Print["FIRST SUMMARY: Dominant behaviors and first constants of "<>
        ...
        Print[
        (* FIX WH 06/30/2018. pdeForm can no longer be applied. *)
        (* replaced pdeForm[CleanUpFunction[...] ] by *)
        CleanUpFunction[ theinitialConstantsOfIntegration ] 
        ];
        *)
        Print[
        pdeForm[ CleanUpFunction[ theinitialConstantsOfIntegration ] ] 
        ];
      Print["*************************************************************"]
    ]; (* end If *)
    ...
    If[painleveTestDebug || (Verbose /. {options} /. Options[PainleveTest]),
      ...
      Print[
      (* FIX WH 06/30/2018. pdeForm can no longer be applied. *)
      (* replaced pdeForm[ CleanUpFunction[ theResonances ] ] by *)
          CleanUpFunction[ theResonances ]
      ];
      *)
       Print[
        pdeForm[ CleanUpFunction[ theResonances ] ]
      ];
      Print["*************************************************************"]
    ]; (* end If *)
...
If[debugwilly1,
   ...
   Print[ CleanUpFunction[ theConstantsOfIntegration ] ]
    ...
    If[painleveTestDebug || (Verbose /. {options} /. Options[PainleveTest]),
      ...
      Print[
      (* FIX WH 06/30/2018. pdeForm can no longer be applied. *)
      (* replaced pdeForm[ CleanUpFunction[ theConstantsOfIntegration ] ] by *)
       CleanUpFunction[ theConstantsOfIntegration ]
      ];
      *) 
      Print[
        pdeForm[ CleanUpFunction[ theConstantsOfIntegration ] ]
      ];
      Print["End of FINAL SUMMARY."];
      (* FIX WH 06/30/2018. Added comment *)
      Print["Results below are in standard Mathematica notation for derivatives"];
      Print["(in case further computations with Mathematica were needed)."];
      Print["*************************************************************"]
    ];
...
If[debugwilly1,
   Print["Entering the function dominantBehavior."]
    ...
    If[dominantBehaviorDebug,
      ...
      Print[
      (* FIX WH 06/30/2018. pdeForm can no longer be applied. *)
      (* replaced pdeForm[ CleanUpFunction[ theSystem ] ] by *)
          CleanUpFunction[ theSystem ]
      ];
      *)
      Print[
       pdeForm[  CleanUpFunction[ theSystem ] ]
      ];
    ]; (* end If *)
    ...
    If[dominantBehaviorDebug,
      ...
      (* FIX WH 06/30/2018. pdeForm can no longer be applied. *)
      (* replaced pdeForm[ CleanUpFunction[ theSystem /. ... ] ] by *)
       CleanUpFunction[ theSystem /. myTrackingVariable[_] -> 1 ]
      ];
      *)
      Print[
       pdeForm [ CleanUpFunction[ theSystem /. myTrackingVariable[_] -> 1 ] ]
      ];
    ]; (* end If *)
...
initialConstantsOfIntegration`GenerateSystem[
  ...
  Print["Entering the function initialConstantsOfIntegration'GenerateSystem."]
    ...
    If[systemGenerationDebug,
      ...
      (* Did not work. FIX WH 06/30/2018 *)
      (* 
      Print["HHHHHHHHHHHHHHHHHH 1, ansatzRules: "];
      Print[ansatzRules];
      Print["HHHHHHHHHHHHHHHHHH 2, FullForm of ansatzRules: "];
      Print[FullForm[ansatzRules]];
      Print["HHHHHHHHHHHHHHHHHH 3, FullForm of ansatzRules under rule {x$ -> xx, t$ -> tt} : "];
      Print[ CleanUpFunction2[ ansatzRules //. {x$ -> xx, t$ -> tt, y$ -> yy, z$ -> zz} ] //. {x$ -> xx, t$ -> tt, y$ -> yy, z$ -> zz} ]; 
      (* FIX WH 06/30/2018. Also replace x$ by x, t$ by t, etc. *)
      stringhereman1 = CleanUpFunction[ ansatzRules ] ;
      stringhereman2 = stringhereman1 /. {x$ -> x, t$ -> t};
      Print["HHHHHHHHHHHHHHHHHH 4, stringhereman2 : "];
      Print[stringhereman2];
      *)
    ];
  If[debugwilly1,
...
resonances[
  ...
  Module[{ resonancesDebug = (* The debug Boolean. *)
    ...
    If[resonancesDebug,
      ...
      (* testing format *)
      (* 
      Print["IIIIIIIIIIIIIIIIIII 1, theSystem: "];
      Print[theSystem];
      Print["IIIIIIIIIIIIIIIIIII 2, FullForm of theSystem: "];
      Print[FullForm[theSystem]]; 
      *)
      (* was not needed FIX WH 06/30/2018. Also replace x$ by x, t$ by t, etc. *)
      (* Print[ CleanUpFunction2[ theSystem ]  //. {x$ -> x, t$ -> t, y$ -> y, z$ -> z}]; *)
    ]; (* end of If *)
...
resonances`GenerateSystem[
  ...
  Module[{ systemGenerationDebug = (* The debug Boolean. *)
    ...
    If[ Verbose /. {options} /. Options[PainleveTest],
      ...
      (* FIX WH 06/30/2018. Change name from resonances`GenerateSystem to resonances? *)
      (* Not done yet *)
      Print["The system (entering the function resonances`GenerateSystem) is: "];
      (* *)
      (* Print["The system (entering the function resonances) is: "]; *)
      (* FIX WH 06/30/2018 was standard format. Changed to pdeForm. Also used proper equation format. *)
      (* Print[ CleanUpFunction[ equations ]  ]; *)
      Print[ pdeForm[ CleanUpFunction[ Map[# ==0&, equations] ] ] ];
    ...
    If[systemGenerationDebug,
      ...
      (*
      Print["The ansatz rules (for computing the resonances) are: "];
      Print["JJJJJJJJJJJJJJJJJ 1, ansatzRules: "];
      Print[ansatzRules];
      Print["JJJJJJJJJJJJJJJJJ 2, FullForm of ansatzRules: "];
      Print[FullForm[ansatzRules]];
      (* Did not work. FIX WH 06/30/2018. Also replace x$ by x, t$ by t, etc. *)
      Print[ CleanUpFunction2[ ansatzRules ] //. {x$ -> x, t$ -> t, y$ -> y, z$ -> z} ];
      *)
    ]; (* end of If *)
    If[debugwilly1,
...
constantsOfIntegration[
  ...
  Module[{ constantsOfIntegrationDebug = (* The debug Boolean. *)
    ...
    If[ Verbose /. {options} /. Options[PainleveTest],
       ...
       Print["START OF A NEW BRANCH OF THE COMPUTATIONS."];
  (* WH:05/23/2008, added details about the branch considered *)
      Print["The system (entering the function constantsOfIntegration) is: "];
      (* FIX WH 06/30/2018 was standard format. Changed to pdeForm. Also used proper equation format. *)
      (* Print[ CleanUpFunction[ equations ]  ]; *)
      Print[ pdeForm[ CleanUpFunction[ Map[# ==0&, equations] ] ] ];
  If[debugwilly1,
...
(* WH:05/31/2007, fix in output added == 0, so it reads expr == 0 *)
    If[ Length[ compatibilityConditions ] > 0,
      ...
      (* FIX WH 06/30/2018. pdeForm can no longer be applied. *)
      (* replaced pdeForm[ CleanUpFunction[ ... ] ] by *)
          CleanUpFunction[ compatibilityConditions], "Message"
      ]; 
      *)
     StylePrint[
        pdeForm[ CleanUpFunction[ compatibilityConditions] ], "Message"
      ];  
   ...
   "inside the constantsOfIntegration`NthResonance`Solve function."]
    ...
    theSolution =
      Solve[
        ...
        uList 
        (* ,
        (* FIX WH 06/30/2018 Removed Sort -> False. Was option of Solve no longer supported starting from Mathematica 8 *)
        (* May require a better solution -- see code Painleve3-2018 based on modifications done by Goktas in 2012 and 2013 *)
        (* Modifications not included yet on June 30, 2018 at 18:33 *)
        Sort -> False 
        *)
      ]; (* end of Solve *)
...
(* HEREMAN CRM Montreal: Trouble is in this function *)
constantsOfIntegration`NthResonance`Solve`IterativelySolve[
    equations_List,
    unknowns_List,
    solutions_List,
...
(* FIX WH 06/30/2018. To also replace x$ by x, y$ by y, z$ by z, and t$ by t *)
CleanUpFunction2 =
  ToExpression[
    StringReplace[ToString[InputForm[# //. {x$ -> x, t$ -> t, y$ -> y, z$ -> z}]],
      "Calculus`PainleveTest`Private`"->""
    ]
  ]&
...
(* FIX WH 06/30/2018. The pdeForm function does no longer work correctly under Mathematica 8 and subsequent versions *)
(* FIX WH 06/30/2018. old version of pdeForm can no longer be applied. *)
...
(* FIX UG 06/18/2012. Subscript problem in V8 *)
(* updated by Goktas to solve the subscripting problem in V8 *)
(* - by Goktas on June 18, 2012, in Golden *)

(* start of new pdeForm *)
pdeForm[expr_] :=
   Module[{notationrules, f},
      notationrules = 
       {
        u[__] -> u, g[__] -> g,
        (* FIX WH 06/30/2018. Added a few rules to replace, e.g., u[x,t] by u itself *)
        u[a_] -> u, v[a_] -> v, w[a_] -> w, 
        u[a_,b_] -> u, v[a_,b_] -> v, w[a_, b_] -> w, 
        u[a_,b_,c_] -> u, v[a_,b_,c_] -> v, w[a_, b_,c_] -> w, 
        u[a_,b_,c_,d_] -> u, v[a_,b_,c_,d_] -> v, w[a_, b_,c_,d_] -> w, 
        Derivative[i__][u_][j__] :> 
                       Subscript[u, StringJoin[StringJoin[ToString /@ #] & /@
           (Thread[f[{j}, {i}]] /. f -> (Table[#1, {#2}] &))]]
       };
      expr /. notationrules
   ]
(* end of new pdeForm *)

(* FIX WH 06/30/2018. Variation of function found on Internet. Does not work with pdeForm *)
(* 
HideArgsWilly[exp_] := With[{exp2 = exp + 0[_]},   
                       Replace[exp2, #[__] -> # & /@  Variables[exp2][[All, 0]], \[Infinity]]]
*)

(* old pdfForm from 2009 used in PainleveTestV2.m *)
(* 
pdeForm[expr_] :=
  expr /.
...
*)
(* end of old pdeForm used in PainleveTestV2 of 2009 *)
...
Print["Package PainleveTestV4.m was successfully loaded."];
(* FIX WH 06/30/2018. Dates updated *)
Print["Last updated by Hereman on Saturday, June 30, 2018 (19:10)."];
Print["Based on PainleveTestV2 from January 2009. Has updated pdeForm function."];
Print["Previously updated by Hereman on Sunday, January 25, 2009."];
