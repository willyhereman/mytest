...
(* *********************************************************************** *)
(* Code updated: Sunday, June 23, 2013 at 12:15p.m. in Ankara by Goktas *)
(* Code previously updated: Monday, June 18, 2012 at 16:35 in Golden *)
(* by Goktas *)
(* Code previously updated: Sunday, January 25, 2009 at 22:10 in Boulder *)
(* by Hereman *)
(* Previously updated on Friday, January 2 at 10:00AM by Baldwin in Arvada *)

(* : Title: PainleveTest.m (Version 3) *)
(* : Authors: Douglas Baldwin, Willy Hereman and Unal Goktas *)
...
(* : Code tested for Mathematica 3.0, 4.0, 4.1, 5.0, 5.1, 6.0, 8.0 and 9.0 *)
...
PainleveTest[
  ...
  Module[{ painleveTestDebug = False, (* The debug Boolean. *)
    ...
    If[{} =!= (KruskalSimplification /. {options} /. Options[PainleveTest]),
      equations =
        Module[{x,temp,temp2},
          (* Sets x as the independent variable used in *) 
          (* KruskalSimplification. *)
...
constantsOfIntegration`NthResonance`Solve`IterativelySolve[
  ...
  Module[{ iterativelySolveDebug = (* The debug Boolean. *)
           ...
           sortoptionwas = 
           SystemOptions["ReduceOptions" -> {"ReorderVariables"}]
           (* instead of Sort -> False option in Solve in V5 which *) 
           (* was obselete in V8 - by Goktas in Golden on June 18, 2012 *)
         }, (* Protected Local Variables *)
SetSystemOptions["ReduceOptions" -> {"ReorderVariables" -> False}];
(* instead of Sort -> False option in Solve in V5 which was obselete in V8 *)
(* - setting the systems options - by Goktas in Golden on June 18, 2012 *)
   If[debugwilly1,
      Print["Entering the function "<>
      "constantsOfIntegration`NthResonance`Solve`IterativelySolve."]
     ];
...
SetSystemOptions[sortoptionwas];
(* instead of Sort -> False option in Solve in V5 which was obselete in V8 *)
(* - setting the systems options back to its original settings - *)
(* by Goktas in Golden on June 18, 2012 *)
      Return[
...
If[debugwilly1,
    ...
    theSolution =
      Solve[
        ...
        Sort -> False*)
(* instead of Sort -> False option in Solve in V5 which was obselete in V8 - 
now using setting system options  - by Goktas in Golden on June 18, 2012*)
      ];
...
(* updated by Goktas to solve the subscrpting problem in V8 *)
(* - on June 18, 2012, in Golden *)
pdeForm[expr_] :=
...
Print["Last updated by Goktas on Sunday, June 23, 2013."];
(* Print["Last updated by Goktas on Monday, June 18, 2012."]; *)
(* Print["Package PainleveTest.m was successfully loaded."]; *)
